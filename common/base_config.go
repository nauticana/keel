...
//                               values per node. The reserved node_id -1 is a
//                               shared fallback: a flag with no row for the
//                               running node inherits the -1 row, so config
//                               identical across nodes is written once, not per
//                               node. Absent both, application_config_flag.
//                               default_value applies.
...
// Resolution precedence per flag: this node's own value → the shared node_id=-1
// fallback value → the catalog default_value. Two LEFT JOINs (n = node row,
// s = shared -1 row) keep every flag even when neither value row exists; COALESCE
// prefers the node row, then the shared row, then leaves NULL so ParseValue*
// applies default_value. A plain (INNER) join would drop unset flags entirely.
//
// The -1 bucket lets a value that is identical across every node be written once
// instead of once per node_id; a node overrides it by inserting its own row.
...
func (c *BaseConfig) ApplyBase(m map[string]ConfigRow) error {
	...
	for _, flg := range baseFlagIDs {
		...
		case sms_provider:
			c.SMSProvider = c.ParseValueS(r.Value, r.Default)
		case sms_service_sid:
			c.SMSServiceSID = c.ParseValueS(r.Value, r.Default)
