...
-- Runtime note: agency commission/provenance services currently require PostgreSQL.
...
-- user_bank_info_active_uq is a partial index on PostgreSQL (WHERE status = 'A'); MySQL cannot enforce it — service-enforced
CREATE INDEX user_bank_info_active_uq ON user_bank_info(user_id, partner_id);
...
CREATE TABLE IF NOT EXISTS invoice (
    ...
    tax                                  DECIMAL(20,4) NOT NULL DEFAULT 0.00,
...
CREATE UNIQUE INDEX invoice_id_partner_uq ON invoice(id, partner_id);
CREATE UNIQUE INDEX invoice_id_currency_uq ON invoice(id, currency);
...
CREATE TABLE IF NOT EXISTS payment_record (
    ...
    CONSTRAINT invoice_payment_records FOREIGN KEY (invoice_id) REFERENCES invoice(id),
    CONSTRAINT payment_record_invoice_partner FOREIGN KEY (invoice_id, partner_id) REFERENCES invoice(id, partner_id),
...
CREATE TABLE IF NOT EXISTS agency_profile (
    ...
    wholesale_allowed                    TINYINT(1)    NOT NULL DEFAULT 0,
    default_commission_rate_bp           INT          ,
    approved_at                          DATETIME     ,
    suspended                            TINYINT(1)    NOT NULL DEFAULT 0,
    created_at                           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at                           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (agency_partner_id),
    CONSTRAINT agency_profile_partner FOREIGN KEY (agency_partner_id) REFERENCES business_partner(id) ON DELETE CASCADE,
...
CREATE INDEX idx_agency_client_invitation ON agency_client_invitation(agency_partner_id, status);
...
CREATE TABLE IF NOT EXISTS agency_client_delegation (
    ...
    revoked_by                           BIGINT       ,
    PRIMARY KEY (id),
    CONSTRAINT delegation_client FOREIGN KEY (client_partner_id) REFERENCES business_partner(id) ON DELETE CASCADE,
    CONSTRAINT delegation_agency FOREIGN KEY (agency_partner_id) REFERENCES agency_profile(agency_partner_id) ON DELETE CASCADE,
    CONSTRAINT delegation_grantor FOREIGN KEY (granted_by) REFERENCES user_account(id) ON DELETE RESTRICT,
    CONSTRAINT delegation_revoker FOREIGN KEY (revoked_by) REFERENCES user_account(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS agency_client_billing (
    ...
    created_at                           DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    CONSTRAINT client_billing_delegation FOREIGN KEY (client_delegation_id) REFERENCES agency_client_delegation(id) ON DELETE CASCADE,
...
-- agency_client_billing_open_uq is a partial index on PostgreSQL (WHERE effective_to IS NULL); MySQL cannot enforce it — service-enforced
CREATE INDEX agency_client_billing_open_uq ON agency_client_billing(client_delegation_id);
CREATE INDEX idx_agency_client_billing_period ON agency_client_billing(client_delegation_id, effective_from);
...
CREATE TABLE IF NOT EXISTS agency_commission (
    ...
    earning_period_start                 DATETIME      NOT NULL,
    earning_period_end                   DATETIME      NOT NULL,
    gross_amount_minor                   BIGINT        NOT NULL,
    commission_amount_minor              BIGINT        NOT NULL,
    applied_rate_bp                      INT           NOT NULL,
    currency                             CHAR(3)       NOT NULL,
    ...
    PRIMARY KEY (id),
    CONSTRAINT commission_client_billing FOREIGN KEY (client_billing_id) REFERENCES agency_client_billing(id),
    CONSTRAINT commission_source_allocation FOREIGN KEY (invoice_line_payment_id) REFERENCES invoice_line_payment(id),
    CONSTRAINT commission_reversal_of FOREIGN KEY (reversal_of_id) REFERENCES agency_commission(id),
...
-- agency_commission_source_uq is a partial index on PostgreSQL (WHERE entry_type = 'E'); MySQL cannot enforce it — service-enforced
CREATE INDEX agency_commission_source_uq ON agency_commission(invoice_line_payment_id, earning_period_start);
CREATE INDEX idx_agency_commission_status ON agency_commission(status, currency);
CREATE INDEX idx_agency_commission_billing ON agency_commission(client_billing_id);
...
CREATE TABLE IF NOT EXISTS agency_payout (
    ...
    selection_cutoff_at                  DATETIME      NOT NULL,
...
CREATE UNIQUE INDEX agency_payout_transfer_uq ON agency_payout(provider, provider_transfer_id);
-- idx_agency_payout_funding is a partial index on PostgreSQL (WHERE provider_funding_id IS NOT NULL); MySQL cannot enforce it — service-enforced
CREATE INDEX idx_agency_payout_funding ON agency_payout(provider_funding_id);
-- agency_payout_cycle_uq is a partial index on PostgreSQL (WHERE status <> 'F'); MySQL cannot enforce it — service-enforced
CREATE INDEX agency_payout_cycle_uq ON agency_payout(agency_partner_id, currency, provider_account_id, selection_cutoff_at);
