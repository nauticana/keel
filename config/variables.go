...
func LoadRows(ctx context.Context, db port.DatabaseRepository, nodeID int) (ConfigRows, error) {
	...
	for _, row := range res.Rows {
		rows[configText(row[0])] = ConfigRow{
			Value:   configText(row[1]),
			Default: configText(row[2]),
			Source:  configText(row[3]),
		}
	}
	if len(rows) == 0 {
...
// ApplyRows parses and publishes the Keel section of an already-loaded catalog.
func ApplyRows(rows ConfigRows) error {
	kc := &KeelConfig{}
	if err := kc.Apply(rows); err != nil {
		return err
	}
	SetConfig(kc)
	return nil
