...
| `dms` | Document model over `storage`: content repositories (`RepositoryDefinition`, `RepositoryCatalog`, `ContentRepositoryService` with `Add`/`Remove`/`Reload`) and ArchiveLink-style documents (`ContentDocumentService`: a header object plus one object per component, metadata in object attributes). Imports no database package |
| `document` | Partner-owned documents over `dms` for applications with a database: `content_repository`, `document_type` and `partner_document` tables, `TableCatalog`, and `DocumentService` (store a new version, read, signed URL, review with supersession, retire, list) |
| `scan` | The content-scanning port `ContentScanner` and its providers: `ScannerClamd` (clamd INSTREAM); `scan.NewFromConfig()` reads `scan_mode` / `scan_addr` |
| `extract` | Text extraction port `TextExtractor` (`Extract(ctx, mediaType, raw) → Extracted{Text, Sections}` with heading (levelled) / paragraph / table / page byte ranges) and `Native`, the pure-Go extractor for plain text, Markdown, DOCX and PDF text layers; `extract.NewFromConfig()` reads `extract_mode` and `extract_max_bytes` |
...
- `storage_mode=s3|gcs|azure|file`
  ...
  - **Cloudflare R2 / S3-compatible**: use `s3` plus `s3_endpoint=https://<account>.r2.cloudflarestorage.com` (this switches the client to path-style addressing). Verify `PutObjectIfAbsent` (`If-None-Match: *`) on the provider before relying on it.
