...
Plan below is still accurate: the `user_partners` FK exists ([schema/tenant_management/partner_user.yml](schema/tenant_management/partner_user.yml)) and `foreign_key_lookup` carries no row for it.

- New DB view `unassigned_user_account` shipped via `schema/core/`.
...
| C6 | Operator replay and dead-letter — `WebhookProcessor.RetryFailed` atomically claims stored failed deliveries, marks replay events, counts attempts, and transitions the final failure to terminal status `L` | [payment/webhook_processor.go](payment/webhook_processor.go), [payment/webhook_repository_sql.go](payment/webhook_repository_sql.go), [schema/payment/payment_webhook_log.yml](schema/payment/payment_webhook_log.yml) |
